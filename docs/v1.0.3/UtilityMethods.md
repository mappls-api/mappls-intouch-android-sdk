...
new Config.Builder(getContext())
        .setPriority(InTouch.BEACON_PRIORITY_FAST)
        .setEnableRequestPermissionIfMissing(true)
        .build();
```
...
1. `setEnableRequestPermissionIfMissing(boolean)`: To Enable Request Permission if permission is missing.
2. `setDisplacement(int)`: To set the minimum distance between location updates
3. `setIsAccelerometerSensorRequired(boolean)`: To enable/disable use of Accelerometer Sensor
4. `setGNSSFullTrackingEnable(boolean)`: To enable GNSS Full Tracking mode
5. `setWaitForAcurateLocation(bbolean)`: If set to true than will delay delivery of initial low accuracy locations for a small amount of time in case a high accuracy location can be delivered instead.
